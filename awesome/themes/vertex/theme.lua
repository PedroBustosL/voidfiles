...
local mywifisig = awful.widget.watch(
    { awful.util.shell, "-c", "awk 'NR==3 {printf(\"%d-%.0f\\n\",$2, $3*10/7)}' /proc/net/wireless; iw dev wlp1s0 link" },
    2,
...
wificon:connect_signal("button::press", function() awful.spawn(string.format("%s --class=wavemon -e wavemon", awful.util.terminal)) end)
